...
const hn_log = std.log.scoped(.hn);
const curl_log = std.log.scoped(.curl);
        ...
        while (self.running.load(.unordered)) {
            ...
            const count = blk: {
                const c = self.multi.perform() catch |err| {
                    curl_log.err("failed to perform : {any}", .{err});
                    continue;
                };
                break :blk c;
            };
...
pub fn init(allocator: Allocator) !Self {
    ...
    const n_jobs = @max(std.Thread.getCpuCount() catch 0, 4);
...
pub fn fetch(self: *Self, allocator: Allocator, url: [:0]const u8) ![]const u8 {
    hn_log.debug("fetching url {s}", .{url});
    defer hn_log.info("fetched url {s}", .{url});
...
pub fn fetchAllItems(
    ...
    for (jobs, 0..) |*job, i| {
        items[i] = job.await(allocator) catch |err| {
            switch (err) {
                Error.FetchError => hn_log.err("failed to fetch item: {d}", .{ids[i]}),
                else => {},
            }
            return err;
        };
            ...
            self.thread_pool.spawnWg(w, struct {
                fn _(hn: *Self, data: []Item) void {
                    hn.db.insertItems(data) catch |err| {
                        hn_log.err("failed to cache items: {any}", .{err});
                    };
                }
            }._, .{ self, items });
        } else {
...
pub fn fetchThreadAlgolia(self: *Self, allocator: Allocator, opID: ItemID) ![]Item {
    hn_log.debug("fetching thread {d} from algolia", .{opID});
    errdefer hn_log.info("failed to fetch thread {d} from algolia", .{opID});
    ...
    hn_log.debug("parsing thread {d} response from algolia", .{opID});
    const parsed = try std.json.parseFromSlice(AlgoliaItem, allocator, json_str, .{
        .ignore_unknown_fields = true,
    });
    defer parsed.deinit();
    hn_log.debug("parsed thread {d} response from algolia:\n{s}...\n", .{ opID, json_str[0..32] });
    ...
    hn_log.info("fetched thread {d} from algolia : got {d} items ", .{ opID, result.items.len });
...
pub fn fetchLineage(_: *Self, allocator: Allocator, item_id: ItemID) ![]Item {
    hn_log.debug("fetching discussion {d} from HN official API", .{item_id});
    ...
    hn_log.info("fetched discussion {d} from HN official API", .{item_id});
    return items;
}
...
pub fn fetchThreadOfficial(self: *Self, allocator: Allocator, opID: ItemID) ![]Item {
    hn_log.debug("fetching thread {d} from HN official API", .{opID});
    var result: std.ArrayList(Item) = .{};
    defer result.deinit(allocator);
    errdefer {
        for (result.items) |item| {
    ...
    hn_log.info("fetched thread {d} from HN official API", .{opID});
...
pub fn fetchDiscussion(
    ...
    hn_log.debug("fetching discussion {d}", .{item_id});
    errdefer hn_log.info("failed to discussion thread {d}", .{item_id});

    var cached_items = try self.db.readDiscussion(allocator, item_id);
    if (cached_items) |items| {
        hn_log.info("fetched cached discussion {d}: found {d} items", .{ item_id, items.len });
        return items;
    }
...
pub fn fetchThread(
    ...
    hn_log.debug("fetching thread {d}", .{opID});
    errdefer hn_log.info("failed to fetch thread {d}", .{opID});

    const cached_items = try self.db.readThread(allocator, opID);
    if (cached_items) |items| {
        hn_log.info("fetched cached thread {d}: found {d} items", .{ opID, items.len });
        return items;
    }
    ...
    hn_log.info("fetched thread {d}: got {d} items", .{ opID, items.len });

    if (options.wg) |wg| {
        self.thread_pool.spawnWg(wg, struct {
            fn _(hn: *Self, op_id: ItemID, data: []Item) void {
                hn.db.insertItems(data) catch |err| {
                    hn_log.err("failed to cache thread: {d}: {any}", .{ op_id, err });
                };
            }
        }._, .{ self, opID, items });
    } else {
...
const DB = struct {
    ...
    pub fn insertFeed(self: @This(), feed: Feed, ids: []const ItemID) !void {
        ...
        defer conn.commit() catch {
            hn_log.err("failed to commit insertFeed: {s}", .{conn.lastError()});
        };
        errdefer conn.rollback();

        errdefer hn_log.err("failed to read item: {s}", .{conn.lastError()});
    ...
    pub fn readFeed(
        ...
        errdefer hn_log.err("failed to read feed: {s}", .{conn.lastError()});
    ...
    pub fn readThread(
        ...
        while (rows.next()) |row| {
            ...
            if (now - item.inserted >= expiration_sec) {
                hn_log.debug("thread cached expired: {d}", .{op_id});
    ...
    pub fn readDiscussion(
        ...
        while (rows.next()) |row| {
            ...
            if (now - item.inserted >= expiration_sec) {
                hn_log.debug("discussion cached expired: {d}", .{item_id});
                for (result.items) |entry| entry.free(allocator);
                result.deinit(allocator);
                return null;
            }
    ...
    pub fn insertItems(self: @This(), items: []Item) !void {
        ...
        errdefer {
            hn_log.err("failed to insert items: {s}", .{conn.lastError()});
        }
    ...
    pub fn createDatabase(self: @This()) !void {
        ...
        defer conn.release();
        errdefer {
            hn_log.err("failed to create database: {s}", .{conn.lastError()});
        }
    ...
    pub fn startCleaner(self: @This()) !void {
        ...
        const thread = try std.Thread.spawn(.{}, struct {
            fn loop(db: Instance) void {
                while (true) {
                    hn_log.debug("removing old threads", .{});
                    db.removePastLimit() catch |err| {
                        hn_log.err("failed to remove old items: {any}", .{err});
                    };
    ...
    pub fn removePastLimit(self: @This()) !void {
        const conn = self.write_pool.acquire();
        defer conn.release();
        errdefer hn_log.err("{s}", .{conn.lastError()});
        ...
        if (opt_row) |row| {
            ...
            hn_log.info("deleted {d} old threads", .{num_deleted});
        }
    }
};
